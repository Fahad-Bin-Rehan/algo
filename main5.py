        ...
        try:
            ...
            for row_number, line in enumerate(lines):
                ...
                for pos in bf_positions:
                    matched_string = text[pos:pos + len(search_term)]
                    # Append the match details including the full line
        ...
        for result in results:
            ...
            for match in result['matches']:
                self.result_text.insert(
                    tk.END,
                    f"Location: (Row: {match['row']}, Col: {match['col']}), Matched Text: '{match['matched_string']}' in Line: '{match['full_string']}'\n"
                )
            self.result_text.insert(tk.END, f"Brute Force Time: {result['brute_force_time']} seconds\n")
            self.result_text.insert(tk.END, f"KMP Time: {result['kmp_time']} seconds\n")
